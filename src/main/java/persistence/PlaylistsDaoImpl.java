...
public class PlaylistsDaoImpl implements PlaylistsDao {
    ...
    public Playlists create(Playlists playlist) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO playlists (username, playlistName, description, isPublic) VALUES (?, ?, ?, ?)",
                Statement.RETURN_GENERATED_KEYS)) {

            ps.setString(1, playlist.getUsername());
    ...
    public List<Playlists> findByUsername(String username) throws SQLException {
        Connection conn = connector.getConnection();
        if (conn == null) {
            throw new SQLException("findByUsername(): Could not establish connection to database.");
        }

        ArrayList<Playlists> playlists = new ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement(
                "SELECT * FROM playlists WHERE username = ? ORDER BY playlistName")) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                log.error("findByUsername(): An issue occurred when running the query or processing the resultset. Exception: ", e.getMessage());
                throw e;
            }
        } catch (SQLException e) {
            log.error("findByUsername() - The SQL query could not be prepared. Exception: ", e.getMessage());
            throw e;
        }
        return playlists;
    }
    ...
    public List<Playlists> getVisiblePlaylists(String username) throws SQLException {
        ...
        ArrayList<Playlists> playlists = new ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement(
                "SELECT * FROM playlists WHERE username = ? OR isPublic = 1 ORDER BY playlistName")) {
            ps.setString(1, username);
